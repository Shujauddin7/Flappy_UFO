...
export async function GET() {
    try {
        ...
        // Fetch all past tournaments (is_active = false) ordered by oldest first
        // This ensures Tournament #1 = oldest (Oct 26), Tournament #2 = next, etc.
        const { data: tournaments, error } = await supabase
            .from('tournaments')
            .select('*')
            .eq('is_active', false)
            .gte('end_time', startDate) // Filter by END time (when tournament finished)
            .order('end_time', { ascending: true }); // Order OLDEST FIRST so array[0] = Tournament #1
